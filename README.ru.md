...
Архитектура:
- **Async EventBus** — pub/sub в главном asyncio-цикле (события speech, vision, tts, tts_eos, emotion)
- **AsyncBrain** — стримит токены LLM, отправляет TTS-чанки на середине предложения (порог 4 слова)
...
- **Система памяти** — SQLite (факты, настроение, дневник) + ChromaDB (семантический поиск)

Возможности:
- **Распознавание лиц** через SCRFD + ArcFace (ONNX) с профилями и описаниями
- **Единое pygame-окно** — лицо + камера PiP + текст VLM (F11 — fullscreen)
- **12 эмоций** — `[emotion:sarcastic,0.8]` с интенсивностью, управляют анимацией лица
- **Lip sync** — анимация открытия/закрытия рта, синхронизирована с аудио
- **Циркадная система** — поведение меняется по времени суток (ночь/утро/день/вечер)
- **Долговременная память** — запоминает факты о людях, извлекает релевантные воспоминания
- **Потоковый TTS** — речь начинается до завершения генерации
- **Удалённый LLM** — поддержка удалённого Ollama через `.env` (OLLAMA_URL, OLLAMA_MODEL)
...
Фото индексируются при запуске. Для удобного сбора:

```bash
python scripts/collect_faces.py creator --count 15
```

Настройте профили лиц в `robot_config.yaml`:
...
Настройка через `.env` (см. `.env.example`):
...
Директива `/no_think` в системном промпте отключает режим thinking в qwen3, снижая TTFT с ~10с до ~1-2с.

#### Дорожная карта

- **Фаза A** (готово): Критические фиксы — TTS, FaceID, sd.wait()
- **Фаза B** (готово): Async-архитектура — EventBus, AsyncBrain, потоковый TTS, Watchdog
- **Фаза C** (готово): Когнитивная переработка — /no_think, 12 эмоций, циркадная система, структурный промпт
- **Фаза D** (готово): Система памяти — SQLite + ChromaDB, извлечение фактов, семантический поиск
- **Фаза E** (план): Метрики, PulseAudio AEC, удалённый ASR/VLM
- **Фаза F** (план): Управление моторами (GPIO, serial), датчики
- **Фаза G** (план): Навигация (SLAM), планирование маршрута

## Конфигурация

Файлы конфигурации:
- `configs/robot_config.yaml` — режим робота (async-движок, рекомендуется)
